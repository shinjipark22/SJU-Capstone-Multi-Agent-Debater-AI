...
def _generate_role_reversal(agent: Dict, prompt: str, reversed_stance: str) -> Tuple[str, str]:
    """write_review 서브그래프로 역할반전 발언 생성."""
    from src.graph.subgraphs import write_review

    result = write_review.invoke({
        "topic": "",
        "agent": agent,
        "expected_stance": reversed_stance,
...
def role_reversal_node(state: DebateState) -> DebateState:
    ...
    final_text, raw = _generate_role_reversal(representative, prompt, reversed_stance)
