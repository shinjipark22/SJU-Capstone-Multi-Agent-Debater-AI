...
아래 패턴 중에서 골라 사용하되, 같은 카테고리 내 논제들은 서로 다른 패턴을 사용하세요:
  패턴A: "[주체]의 [행동]은 [X]가 아닌 [Y]다"          → pro=[X]이다, con=[Y]이다
  패턴D: "[사건]은 [표면적 명분]이 아닌 [숨겨진 의도]의 산물이다"  → pro=[표면적 명분]이다, con=[숨겨진 의도]의 산물이다
  패턴E: "[결과]의 책임은 [A]가 아니라 [B]에 있다"      → pro=[A]의 책임이다, con=[B]의 책임이다
  패턴H: "[X]보다 [Y]가 우선되어야 한다"                → pro=[X] 우선이다, con=[Y] 우선이다

[핵심 규칙] title에는 반드시 두 대립 주장([X]와 [Y])이 모두 명시되어야 한다.
pro와 con은 title 안에 등장한 어구를 그대로 사용해 "~이다"로 마무리한다.
title에 없는 내용을 pro/con에 새로 만들어 쓰는 것은 엄격히 금지한다.

예시 (뉴스 → 논제, pro, con):
  뉴스: "AI가 일자리를 대체한다 vs 새로운 직업 창출"
  title: "AI 기술 확산은 일자리의 대체가 아닌 새로운 경력 형태의 창출이다"
  pro:   "새로운 경력 형태의 창출이다"   ← 논제의 결론(Y)
  con:   "일자리의 대체이다"             ← 기각된 입장(X)

  뉴스: "트럼프, 중국산 수입품에 추가 관세 부과"
  title: "트럼프 관세 정책은 제조업 부활이 아닌 글로벌 스태그플레이션의 산물이다"
  pro:   "글로벌 스태그플레이션의 산물이다"  ← 논제의 결론(Y)
  con:   "제조업 부활이다"                   ← 기각된 입장(X)

  뉴스: "이란 전쟁으로 글로벌 유가 급등"
  title: "유가 폭등의 책임은 이란이 아니라 미국의 중동 개입에 있다"
  pro:   "미국 중동 개입의 책임이다"  ← 논제의 결론(Y)
  con:   "이란의 책임이다"            ← 기각된 입장(X)
...
pro: title 안의 [Y](논제가 주장하는 결론)를 "~이다" 단문(15자 이내)으로
con: title 안의 [X]("아닌" 앞의 기각된 입장)를 "~이다" 단문(15자 이내)으로
...
def _generate_once(
    ...
    try:
        ...
        return [
            ...
            and t.get("pro") and t.get("con")
            and t.get("source_indices") is not None
