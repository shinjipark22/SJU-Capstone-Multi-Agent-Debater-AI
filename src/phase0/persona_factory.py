...
[즉시 목표]
자신의 진영({stance_kr})에 맞는 논리적 주장을 펼치고, 상대방의 논거를 비판적으로 분석하여 반박한다.
...
def create_agents(
    ...
    for idx, (stance, intensity) in enumerate(zip(stance_list, agent_intensities), start=1):
        ...
        system_prompt = _build_system_prompt(agent_id, stance, intensity, topic)
