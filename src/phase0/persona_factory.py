...
def _build_system_prompt(
    ...
    prompt = f"""당신은 {stance_kr} 토론자입니다. ~입니다/~습니다 합쇼체를 사용합니다.

논제: {title}{desc_block}찬성 입장: {pro}
반대 입장: {con}
...
반드시 100% 한국어로만 출력하세요. 영어, 중국어 등 외국어를 절대 섞지 마세요.
...
speech의 구조:
## 인사말 — 자기소개와 논제에 대한 입장을 간결하게 밝힌다.
## 입장 표명 — 핵심 주장을 한 문장으로 명확하게 선언한다.
## 논거 1: (소제목) — 원인 → 메커니즘 → 결과 구조로 근거를 전개한다.
## 논거 2: (소제목) — 다른 각도에서 주장을 보강한다.
## 논거 3: (소제목) — 상대방 예상 반론에 선제 대응한다.
## 결론 — 논거를 종합하고, 핵심 주장을 힘 있게 재확인한다."""
    return prompt.strip()
