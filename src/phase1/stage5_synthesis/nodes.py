...
def _build_proposal_prompt(
    ...
    return f"""[5단계: 최적해 회의 — 합의 도출]
'{topic}'에 대한 토론이 끝났다. 이 단계는 **양측이 수용 가능한 합의점·타협안을 구체적으로 도출**하는 것이 목표다. 새 쟁점만 계속 제기하지 말고, 서로의 논거에서 **공통 기반**을 찾아 앞으로 나아가라.
...
[합의 도출 지시]
- 앞 사람 발언에서 **수용할 수 있는 부분**을 1가지 구체적으로 짚고, 그 위에 **한 걸음 더** 나아간 타협안/조건을 1개 제안하라
- 예: "~점은 수긍합니다. 다만 ~조건을 추가하면 양측 모두가 받아들일 수 있습니다"
- 새로운 반박·새 쟁점만 계속 꺼내지 마라 (이미 논박 단계는 끝났다)
- 같은 지적/우려(재교육 비용, 불평등 등)를 **반복만 하지 마라** — 앞서 다른 에이전트가 이미 언급했으면 발전된 형태로만 쓸 것
- 1~2문장. 대화체. 핵심에 **강조**
- 소제목/번호/목록 금지

[형식]
- 한국어, 합니다체 종결 ("입니다"/"합니다"/"됩니다")
- 자연스러운 대화
...
def _build_finalize_prompt(
    ...
    return f"""[5단계: 최적해 회의 — Round 3 개별 최적해 선언]
'{topic}'에 대해 지금까지의 토론과 2라운드 동안의 합의 논의를 종합하여, **네가 개인적으로 지지하는 최적해를 선언하라**.
...
- 반드시 **"제가 생각하는 최적해는 ~입니다"** 형식으로 시작하라 (Round 3 개인 의견. 통합 최적해가 아님)
- 이어서 "이는 ~조건과 ~보완을 전제로 합니다" 식의 전제도 1~2개 덧붙여라
...
### 반박 시작
제가 생각하는 최적해는 ~입니다. 이는 ~조건과 ~보완을 전제로 합니다. (2~3문장)
### 반박 끝"""
