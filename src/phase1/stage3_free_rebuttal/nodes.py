...
[규칙]
- 반드시 합니다체. 모든 문장을 "~합니다", "~입니다"로 끝내라
- 직전 2턴에서 이미 사용한 핵심 논지·인과 프레임을 다시 쓰지 마라. 반드시 새로운 공격 축을 제시하라
- 근거를 인용하면 반드시 "그래서 이것이 이 논점에서 왜 중요한지" 해석을 붙여라
- 수치를 지어내지 마라
...
{my_block}{ref_block}
반드시 아래 구조로 방어하라:
1) 상대가 질문했으면 그 질문에 먼저 직접 답하라
2) 상대 공격의 논리적 허점이나 과도한 일반화를 지적하라
3) 새로운 근거나 사례로 반격하라 (이전에 쓴 근거 반복 금지)

[규칙]
- 반드시 합니다체. 모든 문장을 "~합니다", "~입니다"로 끝내라
- 근거를 인용하면 "그래서 이것이 왜 중요한지" 해석을 붙여라
