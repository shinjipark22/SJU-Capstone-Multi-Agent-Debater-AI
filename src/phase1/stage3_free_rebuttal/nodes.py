...
[먼저 상대 공격의 핵심 지점 파악]
상대가 인용한 **구체적인 entity·사례·수치** (예: 회사명·제품명·국가·연도·기관 등) 가 있으면 정확히 그것을 짚어라.
일반론으로 우회하지 마라. 상대가 "Claude > GPT-4 안전성" 같은 구체 반례를 던졌으면 "그 사례에 대해" 직접 답해야지, "안전성 외 다른 요인도 있다" 식 회피 금지.

반드시 아래 구조로 방어하라:
1) **상대가 든 구체 사례·entity 를 명시적으로 짚으면서** 1문장으로 직접 답하라 (회피·일반화 금지)
...
- 상대 발언에 등장한 구체 entity 를 무시하고 일반론으로 답하면 실격
