...
_FALLBACK_MSGS = {
    "PRO": [
        "데이터센터 전력 소비는 여전히 증가하고 있으며, 기술 개선만으로는 이를 해결했다고 보기 어렵습니다. 그렇다면 규제 없이 이 문제를 어떻게 해결할 수 있다고 보십니까?",
        "규제 없는 인프라 확충은 환경 비용을 사회에 전가합니다. 기업이 자발적으로 환경 보호에 나선 사례가 있습니까?",
        "기술 발전만으로 환경 문제를 해결할 수 있다는 전제 자체가 검증되지 않았습니다. 인프라 확충의 환경 비용은 누가 부담해야 한다고 보십니까?",
    ],
    "CON": [
        "AI 기술이 에너지 효율성 향상에 실질적으로 기여하고 있습니다. 규제 강화가 실제로 기술 혁신을 촉진한 구체적 사례가 있습니까?",
        "과도한 규제는 산업 경쟁력을 약화시키고 혁신을 저해합니다. 규제만으로 글로벌 경쟁에서 뒤처지는 문제를 어떻게 해결하시겠습니까?",
        "규제만으로는 기술 혁신을 대체할 수 없으며, AI 인프라가 선행되어야 합니다. 환경 규제가 산업 성장을 저해하지 않는다는 근거가 있습니까?",
    ],
}
...
좋은 예) 탄소 배출은 여전히 증가하고 있습니다. 기술 혁신만으로 이를 해결할 수 있다는 근거는 부족합니다. 그렇다면 규제 없이 이 문제를 어떻게 해결하시겠습니까?
