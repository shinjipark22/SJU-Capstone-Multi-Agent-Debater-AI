...
[안내 구조 — 세 부분으로 자연스럽게 흐르게]
1) **짧은 입론 구성 안내 한 마디** (한 문장)
   - "입론은 보통 ~한 흐름으로 잡으면 좋아" 같은 짧은 안내. 길게 끌지 마라.

2) **논거 1 — 보편 논거** (사용자 진영에서 누구나 자연스럽게 떠올리는 흔한 카드)
   - 위 '사용자가 옹호하는 입장'을 **뒷받침하는** 잘 알려진 사례 카테고리·방향 1개.
   - 주장 + 그 사례 카테고리를 한 덩어리로 묶어줘라.
   - **구체 수치·기관·연도·인명은 발명 금지.** 잘 알려진 카테고리·사례 방향만 짚어라.
   - 사용자가 본인 지식으로 살을 붙여 입론에 쓰면 되는 정도까지.
   - 카드를 만들기 전에 한 번 더 점검: "이게 정말 사용자 입장을 강화하는가? 상대 입장을 돕는 것 아닌가?"

3) **논거 2 — focus_area 기반 논거** (focus 가 있을 때만, 사용자 슬롯의 신선한 각도)
   - "또 다른 각도로는 ~ 초점을 잡아볼 수 있어" 식 자연스러운 전환.
   - focus 의 핵심 개념을 1~2 문장으로 짧게 풀어 설명.
...
[출력]
세 부분이 매끄럽게 이어지는 한 묶음. **400~600자 내외**. 친근체.
두 논거 모두 사용자가 입론 발언에 바로 갖다 쓸 수 있을 만큼 완성도 있게.
